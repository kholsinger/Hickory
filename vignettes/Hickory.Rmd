...
In a future release I plan to include an interface to `adegenet`, which will allow data from a variety of widely used formats to be used with `Hickory`. For now `Hickory` reads data from a simple CSV file with each individual for whom data is available on rows and with the collection locality in a column labeled `pop` indicating the and with the remaining columns corresponding to the loci for which data are available. The locus columns can have any name that is legal in a `data.frame` or `tibble`.
...
In this case the posterior mean of  `f` is very samll, 0.005, and the 95% credible interval is (0.002, 0.010), indicating that these data provide very little evidence for inbreeding within populations. The posterior mean of `theta` is relatively small for plants, 0.081, and the 95% credible interval is (0.066, 0.097), indicating a low-moderate amount of differentiation among populations.
...
Output from this analysis is interpreted in the same way as before. In this case, the posterior mean of `f` is moderate 0.10 and its 95% credible interval is very broad (0.014, 0.304). This shouldn't be surprising. There is very little information about within-population inbreeding from dominant markers. The estimate of `theta` is similar to what it was with co-dominant marker data: 0.084 (0.067, 0.105). Notice that the effective sample sizes are a bit smaller and the `Rhat` are not quite as close to 1, but they are still acceptable.
